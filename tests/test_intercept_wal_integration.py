"""Tests that /intercept writes through the WAL, and that the eventual
audit_events row appears once the shipper drains it (WS0).

Two cases that used to live here -- observe mode recording an unenforced deny,
and deny events accumulating session risk -- moved to
tests/test_intercept_observe_and_risk.py: both POSTed a policy to the shared
live server and then asserted the decision, which is order-dependent. What is
left needs a real server and a real shipper, and polls for the row rather than
...
async def observe_mode_agent_with_approved_tools():
    """Unlike an observe-mode agent with approved_tools='[]', this fixture has a
    restricted approved_tools list, so it actually exercises the
    approved_tools gate in intercept.py -- an empty list sidesteps that
    branch entirely."""
    from app.models.database import async_session_factory
